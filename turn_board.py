...
class Turn_board:
    ...
    def player_positie(self, positie, root):
        if self.posities[positie] == "-" and not self.game_over:
            ...
            self.buttons[positie].config(
                text=n.player_char
            )  # maakt van de gekozen plek een button met value van player
        ...
        # error handeling voor als er al gekozen locatie werd gekozen
            ...
            # de foutmeldin zelf
            messagebox.showinfo(
                "Illegal move", "You have selected an already chosen location!"
            )
    ...
    def draw_board(self, root):
        ...
        self.buttons[0] = Button(
            root, text=self.posities[0], command=lambda: self.player_positie(0, root)
        )
        self.buttons[0].grid(row=5, column=0, sticky="nesw")
        self.buttons[1] = Button(
            root, text=self.posities[1], command=lambda: self.player_positie(1, root)
        )
        self.buttons[1].grid(row=5, column=1, sticky="nesw")
        self.buttons[2] = Button(
            root, text=self.posities[2], command=lambda: self.player_positie(2, root)
        )
        self.buttons[2].grid(row=5, column=2, sticky="nesw")

        self.buttons[3] = Button(
            root, text=self.posities[3], command=lambda: self.player_positie(3, root)
        )
        self.buttons[3].grid(row=6, column=0, sticky="nesw")
        self.buttons[4] = Button(
            root, text=self.posities[4], command=lambda: self.player_positie(4, root)
        )
        self.buttons[4].grid(row=6, column=1, sticky="nesw")
        self.buttons[5] = Button(
            root, text=self.posities[5], command=lambda: self.player_positie(5, root)
        )
        self.buttons[5].grid(row=6, column=2, sticky="nesw")

        self.buttons[6] = Button(
            root, text=self.posities[6], command=lambda: self.player_positie(6, root)
        )
        self.buttons[6].grid(row=7, column=0, sticky="nesw")
        self.buttons[7] = Button(
            root, text=self.posities[7], command=lambda: self.player_positie(7, root)
        )
        self.buttons[7].grid(row=7, column=1, sticky="nesw")
        self.buttons[8] = Button(
            root, text=self.posities[8], command=lambda: self.player_positie(8, root)
        )
        self.buttons[8].grid(row=7, column=2, sticky="nesw")

        # restart button nog toegevoegd
        Button(root, text="Restart", command=lambda: self.draw_board(root)).grid(
